...
import { principalUserApi } from '../../../../shared/api/client';
...
export const StaffDetail = () => {
  ...
  const loadUser = useCallback(async () => {
    setLoading(true);
    try {
      const result = await principalUserApi.get(userId);
      setUser(result.data);
    } catch (error) {
  ...
  const handleToggleStatus = async () => {
    ...
    setBusy(true);
    try {
      const res = await principalUserApi.updateStatus(user.id, nextStatus);
      setUser(res.data);
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      const res = await principalUserApi.changePassword(user.id, newPassword);
  ...
  const handleDeleteUser = async () => {
    ...
    setBusy(true);
    try {
      await principalUserApi.delete(user.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-xs font-semibold text-slate-400">
        <Link to="/principal/staff" className="hover:text-slate-600 dark:hover:text-slate-200">
