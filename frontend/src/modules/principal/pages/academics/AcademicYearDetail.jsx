...
import { principalAcademicApi } from '../../../../shared/api/client';
...
export const AcademicYearDetail = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [yearRes, classesRes, allClassesRes, teachersRes] = await Promise.all([
        principalAcademicApi.getYear(yearId),
        principalAcademicApi.yearClasses(yearId),
        principalAcademicApi.classes({ limit: 100 }),
        principalAcademicApi.teachers({ status: 'ACTIVE' }),
      ]);
  ...
  const handleUpdateYear = async (e) => {
    ...
    setYearSaving(true);
    try {
      await principalAcademicApi.updateYear(yearId, yearForm);
  ...
  const confirmDeleteYear = async () => {
    ...
    try {
      await principalAcademicApi.deleteYear(deleteYearTarget.id);
      showToast('Academic year deleted', 'success');
      navigate('/principal/academics/years');
    } catch (error) {
  ...
  const handleAddClass = async (e) => {
    ...
    setSaving(true);
    try {
      await principalAcademicApi.addClassToYear(yearId, selectedClassId);
  ...
  const handleRemoveClass = async () => {
    ...
    try {
      await principalAcademicApi.removeClassFromYear(yearId, removeClassTarget.classId);
  ...
  const handleDeleteSection = async () => {
    ...
    try {
      await principalAcademicApi.deleteSection(deleteSectionTarget.id);
  ...
  const handleCreateSection = async (e) => {
    ...
    setSaving(true);
    try {
      await principalAcademicApi.createSection({
  ...
  const handleSaveEditSection = async (e) => {
    ...
    setSaving(true);
    try {
      await principalAcademicApi.updateSection(editSectionModal.id, {
  ...
  return (
    <div className="space-y-6">
      {/* Breadcrumb */}
      <AcademicBreadcrumb items={[{ label: 'Academic Years', to: '/principal/academics/years' }, { label: year.name }]} />
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            {year.status === 'DRAFT' && (
              <button
                type="button"
                onClick={() => runAction(principalAcademicApi.activateYear, 'Academic year activated')}
            ...
            {year.status === 'ACTIVE' && !year.isCurrent && (
              <>
                <button
                  type="button"
                  onClick={() => runAction(principalAcademicApi.setCurrentYear, 'Set as current year')}
                ...
                <button
                  type="button"
                  onClick={() => runAction(principalAcademicApi.completeYear, 'Academic year marked as completed')}
                ...
                <button
                  type="button"
                  onClick={() => runAction(principalAcademicApi.archiveYear, 'Academic year archived')}
            ...
            {year.status === 'ARCHIVED' && (
              <button
                type="button"
                onClick={() => runAction(principalAcademicApi.unarchiveYear, 'Academic year unarchived')}
        ...
        <form onSubmit={handleAddClass} className="space-y-4">
          ...
          {availableClasses.length === 0 && (
            <p className="text-xs text-slate-500">
              ...
              <Link to="/principal/academics/classes" className="font-bold text-primary hover:underline">
              ...
              <tr key={section.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-colors">
                ...
                <td className="px-3.5 py-2.5 text-right whitespace-nowrap">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <Link
                      to={`/principal/academics/years/${yearId}/sections/${section.id}`}
