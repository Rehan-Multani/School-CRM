...
export const SubjectAssignments = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [yearRes, classRes, sectionRes, subjectRes, teacherRes, assignRes] = await Promise.all([
        ...
        principalAcademicApi.sections({ limit: 1000 }),
        principalAcademicApi.subjects({ limit: 1000 }),
        principalAcademicApi.teachers({ limit: 1000 }),
        principalAcademicApi.allSectionSubjects(),
      ]);
  ...
  const refreshAssignments = useCallback(async () => {
    try {
      const res = await principalAcademicApi.allSectionSubjects();
  ...
  const handleInlineTeacherChange = async (assignmentId, teacherId) => {
    setBusy(true);
    try {
      await principalAcademicApi.updateSectionSubject(assignmentId, {
  ...
  const handleCreateAssignment = async (e) => {
    ...
    setBusy(true);
    try {
      await principalAcademicApi.addSectionSubject(assignForm.sectionId, {
  ...
  const handleSaveEdit = async (e) => {
    ...
    setBusy(true);
    try {
      await principalAcademicApi.updateSectionSubject(editingAssignment.id, {
  ...
  const handleDeleteAssignment = async () => {
    ...
    setBusy(true);
    try {
      await principalAcademicApi.deleteSectionSubject(deleteTarget.id);
    ...
    reader.onload = async (ev) => {
      ...
      for (const row of parsed) {
        try {
          ...
          await principalAcademicApi.addSectionSubject(matchedSection.id, {
