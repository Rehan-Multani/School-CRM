...
const BulkCreateModal = ({ open, onClose, onSaved }) => {
  ...
  const handleSave = async () => {
    ...
    for (const row of valid) {
      try {
        await principalAcademicApi.createYear(row);
        success++;
      } catch {
        failed++;
      }
...
export const AcademicYearsIndex = () => {
  ...
  const loadYears = useCallback(async () => {
    setLoading(true);
    try {
      const result = await principalAcademicApi.years();
  ...
  const handleUpdateYear = async (e) => {
    ...
    setYearSaving(true);
    try {
      await principalAcademicApi.updateYear(editingYear.id, yearForm);
  ...
  const confirmDeleteYear = async () => {
    ...
    try {
      await principalAcademicApi.deleteYear(deleteTarget.id);
                    ...
                    <tr key={year.id} className="transition-colors hover:bg-slate-50/80 dark:hover:bg-slate-900/50">
                      ...
                      <td className="px-3.5 py-3 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end gap-1">
                          <Link
                            to={`/principal/academics/years/${year.id}`}
                          ...
                          {year.status === 'DRAFT' && (
                            <button
                              type="button"
                              onClick={() => runAction(principalAcademicApi.activateYear, year.id, 'Academic year activated')}
                          ...
                          {year.status === 'ACTIVE' && !year.isCurrent && (
                            <>
                              <button
                                type="button"
                                onClick={() => runAction(principalAcademicApi.setCurrentYear, year.id, 'Set as current year')}
                              ...
                              <button
                                type="button"
                                onClick={() => runAction(principalAcademicApi.completeYear, year.id, 'Academic year marked as completed')}
                              ...
                              <button
                                type="button"
                                onClick={() => runAction(principalAcademicApi.archiveYear, year.id, 'Academic year archived')}
                          ...
                          {year.status === 'ARCHIVED' && (
                            <button
                              type="button"
                              onClick={() => runAction(principalAcademicApi.unarchiveYear, year.id, 'Academic year unarchived')}
