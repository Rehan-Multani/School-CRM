...
export const ClassesIndex = () => {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      if (editingClass) {
        await principalAcademicApi.updateClass(editingClass.id, payload);
        showToast('Class updated', 'success');
      } else {
        const res = await principalAcademicApi.createClass(payload);
        const newClass = res.data;
        showToast('Class created', 'success');

        if (form.academicYearId && newClass?.id) {
          await principalAcademicApi.addClassToYear(form.academicYearId, newClass.id);
  ...
  const handleImport = (e) => {
    ...
    reader.onload = async (ev) => {
      ...
      for (const row of parsed) {
        try {
          const res = await principalAcademicApi.createClass({
            ...row,
          ...
          if (selectedYear && res.data?.id) {
            await principalAcademicApi.addClassToYear(selectedYear, res.data.id);
          }
          success++;
        } catch { failed++; }
      }
