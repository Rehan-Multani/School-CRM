...
export const SectionDetail = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [sectionRes, subjectsRes, masterSubjectsRes, teachersRes] = await Promise.all([
        principalAcademicApi.getSection(sectionId),
        principalAcademicApi.sectionSubjects(sectionId),
        principalAcademicApi.subjects({ limit: 100, status: 'ACTIVE' }),
  ...
  const handleAddSubject = async (e) => {
    ...
    setSaving(true);
    try {
      await principalAcademicApi.addSectionSubject(sectionId, {
        ...form,
