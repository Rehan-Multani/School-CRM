...
import { principalStudentApi } from '../../../../shared/api/client';
...
export const StudentDetail = () => {
  ...
  const loadStudent = useCallback(async () => {
    setLoading(true);
    try {
      const result = await principalStudentApi.getStudent(studentId);
  ...
  const handleDelete = async () => {
    setBusy(true);
    try {
      await principalStudentApi.deleteStudent(student.id);
  ...
  if (!student) {
    return (
      <EmptyState
        ...
        action={
          <Link to="/principal/students" className="rounded-xl bg-primary px-4 py-2 text-xs font-bold text-white">
            Back to Students
          </Link>
        }
      />
    ...
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        actions={
          <>
            <Link
              to={`/principal/students?edit=${student.id}`}
