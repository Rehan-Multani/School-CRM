...
export const StudentManagement = () => {
  ...
  const loadYearClasses = useCallback(async (yearId) => {
    ...
    try {
      const result = await principalAcademicApi.yearClasses(yearId);
  ...
  const loadStudents = useCallback(async () => {
    ...
    try {
      ...
      const result = await principalStudentApi.students(params);
  ...
  const openEditModal = async (student) => {
    setSaving(true);
    try {
      const result = await principalStudentApi.getStudent(student.id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingStudent) {
        await principalStudentApi.updateStudent(editingStudent.id, payload);
        showToast('Student updated', 'success');
      } else {
        await principalStudentApi.createStudent(payload);
  ...
  const handleToggleStatus = async (student) => {
    ...
    try {
      await principalStudentApi.updateStudentStatus(student.id, nextStatus);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await principalStudentApi.deleteStudent(deleteTarget.id);
